...
let _ =
  "React element"
  >::: [ ( "can create most simple text element"
           >:- fun () ->
             prepare () ;
       ...
       ; ( "can create original stateless component"
           >:- fun () ->
             prepare () ;
             let component =
               R.Component.make_stateless
                 ...
                 ~render:(fun props ->
                     R.create_dom_element R.Tags.span ~children:[R.text @@ Js.to_string props##.name] )
             in
             let index = Dom_html.getElementById "js" in
             let element =
               R.create_element component
                 ~props:
                   (object%js
       ...
       ; ( "can create original stateful component"
           >:- fun () ->
             prepare () ;
             let component =
                 ...
                 ~spec:
                   R.(
                     component_spec
                       ...
                       (fun this ->
                          R.create_dom_element R.Tags.span
                            ~children:[R.text @@ Js.to_string this##.state##.real_name] ))
             in
             let index = Dom_html.getElementById "js" in
             let element =
               R.create_element component
       ...
       ; ( "can handle some event for stateful component"
           >:- fun () ->
             prepare () ;
             let component =
               R.Component.make_stateful
             ...
             let text = Js.Opt.get index##.textContent option |> Js.to_string in
             let v =
               String.split_on_char '\n' text |> List.filter (fun v -> v <> "") |> Array.of_list
             in
             Lwt.return @@ assert_ok ([|"bar"; "did_mount"|] = v) )
       ; ( "can create element from tag with properties"
           >:- fun () ->
             prepare () ;
             let index = Dom_html.getElementById "js" in
             let element =
               R.create_dom_element R.Tags.span ~props:(R.element_spec ~class_name:"test_class" ())
             in
             R.dom##render element index ;
             let open Lwt.Infix in
             Lwt_js.sleep 0.0
       ...
       ; ( "can handle an event given from properties"
           >:- fun () ->
             prepare () ;
             ...
             let element =
               R.create_dom_element R.Tags.span
                 ~props:
                   (R.element_spec () ~class_name:"test_class" ~on_key_down:(fun e ->
             ...
             >>= (fun () ->
                 ...
                 Lwt_js.sleep 0.1 )
             >>= fun () -> wait )
       ; ( "can create element with undeclared properties"
           >:- fun () ->
             prepare () ;
             let index = Dom_html.getElementById "js" in
             let element =
               R.create_dom_element R.Tags.span
                 ~props:
                   (R.element_spec () ~class_name:"test_class"
                      ~others:
                        (object%js
                          val tabIndex = "0"
                        end))
             ...
             let attr =
               match Js.Opt.to_option attr with
               | None -> failwith ""
               | Some attr -> Js.to_string attr##.value
             in
             let others = cls##.attributes##getNamedItem (Js.string "others") in
             Lwt.return @@ assert_ok ("0" = attr && not (Js.Opt.test others)) )
       ; ( "should not override declared properties from in others property"
           >:- fun () ->
             prepare () ;
             let index = Dom_html.getElementById "js" in
             let element =
       ...
       ; ( "can specify key to components in children that are DOM element"
           >:- fun () ->
             prepare () ;
             let index = Dom_html.getElementById "js" in
             let element =
               R.create_dom_element R.Tags.div
                 ~props:(R.element_spec ~class_name:"test_class" ())
                 ~children:
       ...
       ; ( "can specify key to original components in children"
           >:- fun () ->
             prepare () ;
             let component =
               R.Component.make_stateless
             ...
             let element =
               R.create_dom_element R.Tags.div
                 ~props:(R.element_spec ~class_name:"test_class" ())
                 ~children:
                   [ R.create_element ~key:"foo"
                       ~props:
                         (object%js
                           val name = Js.string "foo"
                         end)
                       component
                   ; R.create_element ~key:"bar"
                       ~props:
                         (object%js
                           val name = Js.string "bar"
                         end)
                       component
                   ; R.create_element ~key:"baz"
                       ~props:
                         (object%js
                           val name = Js.string "baz"
                         end)
                       component ]
             in
             R.dom##render element index ;
             ...
             let nodes = cls##.childNodes##.length in
             Lwt.return @@ assert_eq 3 nodes )
       ; ( "can wrap elements by fragment component"
           >:- fun () ->
             prepare () ;
             let component =
               R.Component.make_stateless
       ...
       ; ( "can fetch reference of DOM element"
           >:- fun () ->
             prepare () ;
             let component =
               R.Component.make_stateful
                   ...
                   R.(
                     component_spec
                       ~initial_custom:(fun _ _ ->
                           let refs = Ref_table.create () in
                           Ref_table.define ~key:"node" refs ;
                           object%js
                             val refs = refs
                           end )
                       ~component_did_mount:(fun this ->
                           match Ref_table.find this##.custom##.refs ~key:"node" with
                           | None -> ()
                           | Some e ->
                       ...
                       (fun this ->
                          ...
                          R.create_dom_element R.Tags.span
                            ~_ref:(R.Ref_table.use this##.custom##.refs ~key:"node")
             ...
             let option () = Js.string "" in
             let text =
               Js.Opt.get (index##getAttribute (Js.string "data-test")) option |> Js.to_string
             in
             Lwt.return @@ assert_ok ("value" = text) )
       ; ( "can get children in component"
           >:- fun () ->
             prepare () ;
             let component =
               R.Component.make_stateful
             ...
             let element =
               R.create_element component
                 ...
                 ~children:
                   ( Array.init 3 (fun i -> R.create_dom_element ~key:(string_of_int i) R.Tags.span)
                     |> Array.to_list )
             in
             R.dom##render element index ;
             let open Lwt.Infix in
       ...
       ; ( "can define custom object in this"
           >:- fun () ->
             prepare () ;
             let component =
               R.Component.make_stateful
                 ...
                 ~spec:
                   R.(
                     component_spec
                       ...
                       (fun this ->
                          R.create_dom_element R.Tags.span ~children:[R.text this##.custom##.v] ))
             in
             let index = Dom_html.getElementById "js" in
             let element =
               R.create_element component
