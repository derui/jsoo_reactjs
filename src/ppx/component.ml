...
let build_args loc args =
  let children = ref None and props = ref [] in
  List.iter
    (fun (labelled, exp) ->
      match labelled with
      | (Ppxlib_ast.Asttypes.Labelled _ | Optional _) as v -> props := Some (v, exp) :: !props
...
let ext =
  Extension.declare "c" Extension.Context.expression
